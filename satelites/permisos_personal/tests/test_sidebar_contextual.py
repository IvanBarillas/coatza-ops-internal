...
CARPETA = Path(__file__).resolve().parent.parent / "templates" / "permisos_personal" / "workbench"


class SidebarContextualTests(BasePermisos):
    def test_ningun_parcial_declara_su_propio_aside(self):
        parciales = sorted(CARPETA.glob("*.html"))
        self.assertEqual(len(parciales), 8)
        for parcial in parciales:
            contenido = parcial.read_text()
            self.assertNotIn('id="module-sidebar"', contenido, parcial.name)
            self.assertIn('{% include "shell/_module_sidebar.html" with sidebar_template="permisos_personal/contextual/sidebar.html" %}', contenido, parcial.name)

    def test_el_swap_del_workbench_trae_el_boton_de_contraer(self):
        self.client.force_login(self.ana.usuario)
        parcial = self.client.get(reverse("permisos_personal:mis_solicitudes"), HTTP_HX_REQUEST="true", HTTP_HX_TARGET="workbench")
