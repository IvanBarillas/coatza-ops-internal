    ...
    if request.method == 'POST' and not request.user.is_email_verified:
        with transaction.atomic():
            ...
            if user.email_verification_sent_at and (timezone.now() - user.email_verification_sent_at).total_seconds() < 60:
                ...
                try:
                    send_mail('Confirma tu correo — Axentra OS', f'Confirma tu correo desde tu sesión: {url}\nEl enlace vence en 30 minutos.', settings.DEFAULT_FROM_EMAIL, [user.email], fail_silently=False)
                except Exception:
...
def account_security_view(request):
    return render(request, 'registration/account_security.html', {'title': 'Seguridad de mi cuenta'})
